...
1. **Instagram handle çelişkisi.** Sitenin linklediği `@eryamansiringayrimenkul` Google'da SIFIR görünürlük; indekste ise "Eryaman Şirin Gayrimenkul (`@sirin.gayrimenkul`)" adlı hesap var. Ya hesap yeniden adlandırıldı ya iki hesap var. Uygulamadan doğrula; canlı handle neyse `lib/site-config.ts:56` birebir ona linklesin; mükerrer hesap varsa kapat/birleştir.
2. **Facebook: verilen sayfa ID'si hiç indeksli değil.** `profile.php?id=61585267540417` Google'da yok; indekste `facebook.com/siringayrimenkull/` ("Şirin Gayrimenkul | Ankara", ~9 beğeni) var. Sayfaya kullanıcı adı (vanity URL) al — `profile.php?id=` biçimi arama görünürlüğü üretmiyor; `siringayrimenkull` bizim sayfamızsa site-config linkini ona çevir, ayrı eski sayfaysa birleştir (NAP tutarlılığı). Planlanan "Şirin Gayrimenkul — Eryaman Emlakçı" sayfa adı (platform-profil-metinleri.md §3) dışarıdan hâlâ görünmüyor — uygulanma durumunu panelden kontrol et.
